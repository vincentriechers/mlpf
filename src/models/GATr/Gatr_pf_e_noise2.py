...
class ExampleWrapper(L.LightningModule):
    ...
    def forward(self, g, y, step_count, eval="", return_train=False,use_gt_clusters=False):
        if not use_gt_clusters:
            ...
            scalars = torch.cat((g.ndata["e_hits"].float()+ g.ndata["p_hits"].float(), torch.log(g.ndata["e_hits"].float()+ g.ndata["p_hits"].float()) ), dim=1)
            scalar_type =   g.ndata['h'][:,4:8]
            scalars = torch.cat((scalars, scalar_type, ), dim=1)
