...
if [ -z "$CLDGEO" ]; then
    echo "Will use default CLD geometry version CLD_o3_v01"
    CLDGEO=CLD_o3_v01
else
...
ddsim --compactFile $K4GEO/FCCee/CLD/compact/$CLDGEO/$CLDGEO.xml --outputFile out_sim_edm4hep.root --steeringFile cld_arc_steer.py --inputFiles events.hepmc --numberOfEvents ${NEV} --random.seed ${SEED}
